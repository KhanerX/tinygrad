...
def fsdp(obj, devices: tuple[str]):
  for name, param in nn.state.get_state_dict(obj).items():
    if(param.shape[0] == 1 or prod(param.shape) <= 1) or True:
      param.to_(devices)
    else:
...
class GPT:
  ...
  def load_pretrained(self):
    weights = nn.state.torch_load(fetch(f'https://huggingface.co/gpt2/resolve/main/pytorch_model.bin')) #Loading Large model that OOMs on 12GB VRAM with BS 16 without FSDP
    transposed = ('attn.c_attn.weight', 'attn.c_proj.weight', 'mlp.c_fc.weight', 'mlp.c_proj.weight')
    imported_weights = {} 
    for k in weights:
      imported_weights[k] = weights[k]
      if k == "wte.weight":
        imported_weights[k] = weights[k].pad(((0, self.config.padded_vocab_size-self.config.vocab_size), (0,0))).to(None).contiguous()
      if k.endswith(transposed):
        imported_weights[k] = weights[k].to(None).T.contiguous()
      if "c_attn" in k:
        imported_weights[k.replace("c_attn", "q")] = imported_weights[k].split(self.config.n_embd, dim=0)[0]
        imported_weights[k.replace("c_attn", "k")] = imported_weights[k].split(self.config.n_embd, dim=0)[1]
        imported_weights[k.replace("c_attn", "v")] = imported_weights[k].split(self.config.n_embd, dim=0)[2]
    # lm head and wte are tied
    imported_weights['lm_head.weight'] = imported_weights['wte.weight']
    nn.state.load_state_dict(self, imported_weights)
  ...
  def __call__(self, idx:Tensor, targets=None):
    ...
    if targets is not None:
      logits = self.lm_head(x)[:, :, :self.config.padded_vocab_size]
      loss = logits.sparse_categorical_crossentropy(targets)
    else:
      logits = self.lm_head(x[:, [-1], :])[:, :, :self.config.padded_vocab_size]
      loss = None
...
if __name__ == "__main__":
  ...
  if not args.skip_test:
    ...
    x = (Tensor(start_ids)[None, ...]).expand(2, -1).shard_(GPUS, axis=0)
    max_new_tokens = 16
    temperature = 1.0
    top_k = 40
